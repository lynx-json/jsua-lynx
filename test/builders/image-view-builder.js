...
describe("builders / imageViewBuilder", function () {
  ...
  it("should return view for 'image' with 'data'", function () {
    ...
    return contents.imageViewBuilder(node).then(function (view) {
      ...
      view.children.item(0).height.should.equal(node.value.height);
      view.children.item(0).getAttribute("data-lynx-width").should.equal(node.value.width.toString());
      view.children.item(0).width.should.equal(node.value.width);
  ...
  it("should return view for 'image' with 'data' and no dimensions set", function () {
    var node = {
      base: "http://example.com",
      spec: {
        hints: [{ name: "image" }, { name: "content" }]
    ...
    return contents.imageViewBuilder(node).then(function (view) {
      ...
      view.children.length.should.equal(1);
      view.children.item(0).getAttribute("alt").should.equal(node.value.alt);
      expect(view.children.item(0).getAttribute("data-lynx-height")).to.be.null;
      expect(view.children.item(0).height).to.be.undefined;
      expect(view.children.item(0).getAttribute("data-lynx-width")).to.be.null;
      expect(view.children.item(0).width).to.be.undefined;
      expect(buildStub.called).to.equal(true);
    });
  });
